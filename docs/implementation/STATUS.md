...
| P01 | in_progress | Native English QML shell, CMake modules, commands, immutable snapshots, undo/redo and validation. Typed character/peg/part property addresses separate rest, authored and evaluated values; format-4 migration preserves older scenes with source-version backups. Integrated resizable timing/curve workspace and selection-owned object/layer Properties. Shared compact controls, optional curve numeric fields and a draggable workspace divider with dynamic bounds. | Docking/workspace persistence, configurable shortcuts, full accessibility, scalable resource/cache protocols and cross-platform qualification. |
...
| P08 | in_progress | Parented layers and typed character roots, pegs and parts; role editing, preserve-world rigid reparenting, stable rest pivots, named held substitutions and atomic save/reopen. | Thumbnail browsing, coordinated view sets, instance-safe character duplication, masked poses, Quick Rig, declarative controls, asset/template library and animator validation. |
...
- HM-00 rigid baseline: 19 original sRGB PNG parts form a parented scene saved in format 4; save/reopen is semantically identical and the rendered first frame matches the checked-in 1080p reference. Generated dialogue cues and ten-minute drift markers pass exact sample-boundary checks. This verifies the registered-art baseline; separate HM-03 tests cover initial rigid substitutions, not a completed character shot.
