...
| Partial / HM-04-A | Typed graph, Display/Write, alpha matte, opt-in linear-sRGB profile, bounded revision cache, hierarchy-aware invalidation and cancellation. Speculative next-frame publication now uses immutable snapshots; original 19-part artwork has alpha/output and cost evidence | HM-01 | Run timed native playback/scrub, fuller alpha/color charts and a denser animated production workload before accepting HM-04 | HM-03; HM-10 is another eligible branch when a slot opens |
